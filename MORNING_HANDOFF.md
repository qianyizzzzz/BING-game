...
- 新增初版 `battleDirector`，让 `TurnAnimation`、牌桌 metadata 和 `TableScene3D` 共享同一套 active cue、camera cue、hit-stop 与 VFX 数据。
...
3. 扩展 BattleDirector 到座位反馈和技能 VFX 分层，让 active cue 直接控制目标座位强调。
