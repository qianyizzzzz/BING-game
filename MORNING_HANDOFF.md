...
- 扩展 `npm run test:assets`，覆盖 GLB scene graph、动作剪影、face-detail、turnaround、rig-guide、material QA 和 PBR 三贴图；LOD0 蒙皮不完整和 LOD0/LOD1 动画 clip 缺失仍作为 WIP warning 输出。
- 新增逐角色浏览器验收脚本，覆盖角色选择、房间状态、LOD1 GLB 请求和 3D canvas 采样。
- 新增初版 `battleDirector`，让 `TurnAnimation`、牌桌 metadata、目标座位高亮和 `TableScene3D` 共享同一套 active cue、camera cue、hit-stop 与 VFX 数据。
...
- `npm run test:ui-agents`：通过。
  - 最新报告：`artifacts/playtests/ui-agents-2026-06-13T02-52-37-039Z/report.md`
  - 双玩家 happy path 读到 BattleDirector cue、3D canvas、GLB、目标预览和遮挡检查。
...
3. 继续扩展 BattleDirector 到技能 VFX 分层和音效资源，让 active cue 直接控制粒子层与音频资源。
4. 扩展平衡测试，按 2/3/4/6 人统计平均回合数、防御成功率、反弹收益和群攻收益。
5. 继续把 LOD0 rigid skin 和 LOD1 的预览 clips 升级成运行时可播放的蒙皮动画。
