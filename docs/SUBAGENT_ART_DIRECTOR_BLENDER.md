...
- 每次重大改动后渲染 front、side、three-quarter、table-scale、portrait-crop、action-idle、action-attack、action-defend、action-skill、action-hit。
- 当前脚本必须至少输出 `{characterId}.glb`、`{characterId}-lod1.glb`、`portrait.png`、`mobile-avatar.png`、`turnaround-front.png`、`turnaround-side.png`、`turnaround-three-quarter.png`、`table-scale.png`、5 张动作 QA 图、PBR PNG 贴图、`material-qa.png` 和 `docs/CHARACTER_ASSET_AUDIT.md`。
...
如果 Blender MCP 可用，对每个角色执行：角色 brief -> Blender 初模 -> 半写实材质与比例优化 -> 桌面/头像/移动端头像可读性检查 -> 动作剪影 QA -> LOD 与性能优化 -> 导出 LOD0/LOD1 glb、portrait、mobile-avatar、turnaround、table-scale、action poses -> 更新 docs/CHARACTER_ASSET_AUDIT.md。
