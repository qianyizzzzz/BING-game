...
- 导入参考图和现有占位资产。
- 渲染预览图：正面、侧面、3/4、桌面视角、头像裁切、面部近景。
...
5. **动作剪影 QA**
   ...
   至少输出待机、攻击、防御、技能、受击、倒地 6 张动作剪影图。程序化阶段可以用临时姿态变换；正式资产必须升级为骨骼绑定和可播放动画。
...
8. **导出与命名**
   ...
   apps/client/public/assets/characters/{characterId}/action-down.png
   apps/client/public/assets/characters/{characterId}/skin-preview-attack.png
   apps/client/public/assets/characters/{characterId}/skin-preview-skill.png
   apps/client/public/assets/characters/{characterId}/skin-preview-hit.png
   apps/client/public/assets/characters/{characterId}/skin-preview-down.png
   apps/client/public/assets/characters/{characterId}/rig-guide.png
   apps/client/public/assets/characters/{characterId}/face-detail.png
...
- 每个角色必须有 guide armature 和 `rig-guide.png`，LOD0/LOD1 GLB 必须通过 skinned mesh 与 animation name 检查；`skin-preview-*` 里的琥珀骨架是目标姿态 overlay，报告里要说明是 first-pass blended skin weights、精细权重绘制中，还是已可播放。
- 程序化阶段至少为 `skin`、`cloth`、`leather`、`metal`、`hair` 增加 micro-bump 与 roughness variation，并导出 PNG 贴图；最终资产再替换为高模/手工烘焙 PBR 贴图。
- 每次重大改动后渲染 front、side、three-quarter、table-scale、portrait-crop、mobile-avatar、face-detail、action-idle、action-attack、action-defend、action-skill、action-hit、action-down、skin-preview-attack、skin-preview-skill、skin-preview-hit、skin-preview-down、rig-guide。
- 当前脚本必须至少输出 `{characterId}.glb`、`{characterId}-lod1.glb`、`portrait.png`、`mobile-avatar.png`、`turnaround-front.png`、`turnaround-side.png`、`turnaround-three-quarter.png`、`table-scale.png`、`face-detail.png`、6 张动作 QA 图、4 张蒙皮动作 QA 图、`rig-guide.png`、PBR PNG 贴图、`material-qa.png` 和 `docs/CHARACTER_ASSET_AUDIT.md`。
...
如果 Blender MCP 可用，对每个角色执行：角色 brief -> Blender 初模 -> 半写实材质与比例优化 -> 桌面/头像/移动端头像/面部近景可读性检查 -> 动作剪影 QA -> guide armature / rig-guide QA -> LOD 与性能优化 -> 导出带 skin/animation clips 的 LOD0/LOD1 glb、portrait、mobile-avatar、face-detail、turnaround、table-scale、action poses、rig guide -> 更新 docs/CHARACTER_ASSET_AUDIT.md。
