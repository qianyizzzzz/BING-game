...
- 每个角色必须有 guide armature 和 `rig-guide.png`，LOD0 GLB 必须通过 skinned mesh 检查；`skin-preview-*` 里的琥珀骨架是目标姿态 overlay，报告里要说明是 first-pass rigid skin weights、精细权重绘制中，还是已可播放。
...
如果 Blender MCP 可用，对每个角色执行：角色 brief -> Blender 初模 -> 半写实材质与比例优化 -> 桌面/头像/移动端头像/面部近景可读性检查 -> 动作剪影 QA -> guide armature / rig-guide QA -> LOD 与性能优化 -> 导出 LOD0/LOD1 glb、portrait、mobile-avatar、face-detail、turnaround、table-scale、action poses、rig guide -> 更新 docs/CHARACTER_ASSET_AUDIT.md。
