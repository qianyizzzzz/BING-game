...
- 每个角色必须有 guide armature 和 `rig-guide.png`，LOD0 GLB 必须通过 skinned mesh 检查；报告里要说明是 first-pass rigid skin weights、精细权重绘制中，还是已可播放。
- 程序化阶段至少为 `skin`、`cloth`、`leather`、`metal`、`hair` 增加 micro-bump 与 roughness variation，并导出 PNG 贴图；最终资产再替换为高模/手工烘焙 PBR 贴图。
- 每次重大改动后渲染 front、side、three-quarter、table-scale、portrait-crop、mobile-avatar、face-detail、action-idle、action-attack、action-defend、action-skill、action-hit、action-down、rig-guide。
- 当前脚本必须至少输出 `{characterId}.glb`、`{characterId}-lod1.glb`、`portrait.png`、`mobile-avatar.png`、`turnaround-front.png`、`turnaround-side.png`、`turnaround-three-quarter.png`、`table-scale.png`、`face-detail.png`、6 张动作 QA 图、`rig-guide.png`、PBR PNG 贴图、`material-qa.png` 和 `docs/CHARACTER_ASSET_AUDIT.md`。
