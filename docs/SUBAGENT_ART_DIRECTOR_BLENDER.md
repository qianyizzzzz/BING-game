...
- 每个角色必须有 guide armature 和 `rig-guide.png`，LOD0/LOD1 GLB 必须通过 skinned mesh 与 animation name 检查；`skin-preview-*` 里的琥珀骨架是目标姿态 overlay，报告里要说明是 first-pass rigid skin weights、精细权重绘制中，还是已可播放。
