...
- `battlePresentation` / `BattleDirector` 数据层从 `GameEvent` 生成 `beat`、来源、目标、时间、强度、VFX、SFX 和相机 cue，已供 `TurnAnimation`、`PlayerSeat`、`TableScene3D`、`SkillEffectLayer` 和新手结算摘要共用；结算 readout 会从同回合事件汇总血/饼变化；下一步接音效资源、移动端主指令条和更完整的竞技读局层。
