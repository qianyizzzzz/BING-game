...
- 扩展 UI agent：覆盖 6 个角色选择、GLB 加载、移动端首屏提交区、结算白话摘要。
- 增加 balance/selfplay 指标：2/3/4/6 人局平均回合数、防御成功率、反弹收益、群攻收益。
- 更新 `docs/CHARACTER_ASSET_AUDIT.md`：说明 LOD1 已接入桌面展示，但还没有可播放蒙皮动画。
...
- 增加 GitHub Actions，跑 typecheck、规则回归、turn timeline 和 UI agents。
...
- 当前 LOD1 GLB 只用于桌面展示，还没有运行时动画 clips。
- UI agent 仍偏 happy path，对复杂技能链、断线重连和多人政治局覆盖不足。
