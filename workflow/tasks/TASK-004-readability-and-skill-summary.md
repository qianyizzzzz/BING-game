...
- 已给桌面结算摘要增加“行动者”和血/饼变化 chip，并暴露 `data-resource-delta-count` / `data-resource-deltas` / `data-resource-delta-details`；默认 UI agents 与火箭复杂技能 smoke 已纳入门禁：`artifacts/playtests/ui-agents-2026-06-14T01-01-35-676Z/report.md`。
- 已给每个血/饼变化 chip 暴露 `data-resource-delta-reasons`，默认 UI agents 已验证“招式消耗、出饼、杀、轮结束清零”等原因不会从结算摘要中丢失。
