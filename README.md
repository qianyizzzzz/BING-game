...
| 游戏 HUD | 行动面板显示“下一步 / 目标 / 状态”，提交后显示已锁定行动，多目标技能会在“当前选择”里写全目标，底部固定“沿用上回合 / 提交”。 |
...
| `npm run verify` | 构建一次后运行核心检查与 UI agents。 |
| `npm run verify:release` | 构建一次后运行发布前完整门禁：CI、发布资产审计、默认 UI、复杂技能和角色浏览器。 |
