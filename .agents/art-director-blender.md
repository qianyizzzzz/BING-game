...
- 每次建模后保存 `.blend` 源文件，并导出 LOD0 / LOD1 `.glb`、头像、移动端头像、turnaround、table-scale、face-detail、action pose、skin-preview、rig-guide 和材质 QA。
- LOD0/LOD1 角色都必须能通过资产审计中的 skinned mesh 和 animation name 检查：GLB 里需要有 skin、skinned nodes、JOINTS_0、WEIGHTS_0，以及 `idle / attack / defend / skill / hit / down` clips。
...
- `skin-preview-attack / skill / hit / down` 必须来自骨骼姿态和 skinned mesh；图中的琥珀骨架是目标姿态 overlay，用来检查 mesh 跟随、穿插、折断感和动作可读性。
- 在正式精修前，至少保证 LOD0/LOD1 GLB 内 animation names 可被引擎读取，并且都有 first-pass skin weights；最终目标是精细权重绘制、自然过渡和运行时精修动画。
